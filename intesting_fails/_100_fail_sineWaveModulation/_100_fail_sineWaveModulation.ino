/*
  ...
  Frequency Modulation not really working correct but sounding very 
  interesting
...
void loop()
{
  ...
  startTime = t;
  AmplitudeModulationFrequency_Hz = analogReadScaled(POTI_LEFT) * 0.1;
  temp = ( sin( 2 * PI * t * 0.001 * AmplitudeModulationFrequency_Hz ) + 1 ) * 0.5; // 0..1
  setAmplitude( temp * 127);
  FrequencyModulationFrequency_Hz = analogReadScaled(POTI_RIGHT) * 0.1;

  temp = ( sin( 2 * PI * t * 0.001 * FrequencyModulationFrequency_Hz ) + 1 ) * 0.5; // 0..1

  float signalFrequency_Hz = temp * 1500 + 300;
